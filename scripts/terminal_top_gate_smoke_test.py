...
FIXTURE = ROOT / "tests" / "fixtures" / "cycle_000011"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Run the exact cycle-000011 regression that previously discarded "
            "strong PLUS geometry because its terminal-top lock was 73.8%."
        )
    )
    parser.add_argument(
        "--json",
    ...
    for terminal in terminals:
        ...
        payload["terminals"][terminal.key] = {
            ...
            "terminal_top_acceptance": classification.metrics.get(
                "terminal_top_acceptance"
            ),
            "terminal_top_gate_reason": classification.metrics.get(
                "terminal_top_gate_reason"
            ),
            "geometry_confidence": classification.metrics.get(
                "geometry_confidence"
            ),
            "template_confirmation": classification.metrics.get(
                "geometry_template_confirmation"
            ),
    ...
    if args.json:
        ...
        print("Cycle 000011 conditional terminal-top gate regression")
        ...
        for key, result in payload["terminals"].items():
            print(
                ...
                f"gate={result['terminal_top_acceptance']}  "
